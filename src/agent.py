...
def call_query_planner(
    ...
    system_prompt = (
        ...
        "TRUNCATE, or ALTER. Return ONLY the SQL query text, no explanation, "
        "no markdown fences."
    )
    user_content = (
    ...
    return _strip_code_fences(response.content[0].text).removeprefix("sql\n")
...
def call_error_corrector(
    ...
    system_prompt = (
        ...
        "row-level data with no GROUP BY or aggregate functions, and must "
        "include every required column by its exact name with no aliasing. "
